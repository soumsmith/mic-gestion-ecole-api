...
@ApplicationScoped
public class eleveService implements PanacheRepositoryBase<eleve, Integer> {
    @Inject
    EntityManager em;
    @Inject ecoleDto ecoleDto;
    public  List<eleve> findAlleleveAndClasse(){
         return   em.createQuery("select distinct o from eleve  o join  o.classe c ").getResultList();
    }


    public  List<eleve> findAlleleveAndClasseByClasse( Integer idclass){
        return   em.createQuery("select distinct o from eleve  o join  o.classe c where  c.classeid=: idclass")
                .setParameter("idclass",idclass)
                 .getResultList();
    }

    public  eleve findEleveInfoByEleve ( Integer idEleve){
        return (eleve) em.createQuery("select distinct o from eleve  o join  o.details_infos_eleves c where o.eleveid=: idEleve")
    ...
    public  List<ecole> findByIdTypegroupe_ecole(Long idniv){
        ...
        return    em.createQuery("select o from ecole o join fetch o.groupe_ecole h where h.groupe_ecoleid =:idniv")
