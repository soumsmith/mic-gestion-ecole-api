...
public class EvaluationPassageResource {
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Tag(name = "EvaluationLoader")
	public Response getByClasseAndMatiere(@QueryParam("anneeId") Long anneeId, @QueryParam("classeId") String classeId,
			@QueryParam("matiereId") String matiereId) {

		return Response.ok().entity(
				evaluationService.getByClasseAndMatiere(anneeId, Long.parseLong(classeId), Long.parseLong(matiereId)))
				.build();
	}
	...
	public Response getByClasseAndMatierePeriode(@QueryParam("classeId") String classeId,
		...
		return Response.ok().entity(evaluationService.getByClasseAndMatiereAndPeriode(Long.parseLong(classeId),
				Long.parseLong(matiereId), Long.parseLong(periodeId), Long.parseLong(anneeId))).build();
	}
	...
	@POST
	@Path("/apply-data/{matiere}/{periode}/{annee}/{type}/{notesur}/{user}")
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.TEXT_PLAIN)
	@Tag(name = "EvaluationLoader")
	public Response appliquer(List<EvaluationLoader> evaLoaders, @PathParam("matiere") Long matiere,
			@PathParam("periode") Long periode, @PathParam("annee") Long annee, @PathParam("type") Long type, @PathParam("notesur") String notesur,  @PathParam("user") String user,
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Tag(name = "EvaluationLoader")
	public Response importation(List<ImportEvaluationDto> importEvaluationDtos, @PathParam("classe") Long classe,
			@PathParam("annee") Long annee) {
		String uuid = null;
		try {
...
//    		System.out.println(g.toJson(importEvaluationDtos));
			...
			return Response.serverError()
					.entity(new Message("Info", "Opération d'importation", "Importation bien effectuée")).build();
		}
