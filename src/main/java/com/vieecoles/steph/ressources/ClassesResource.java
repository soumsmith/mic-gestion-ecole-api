...
//	    	System.out.println("ClassesResource.listPopulateByEcole()");
	...
	@GET
	@Path("/get-visible-by-enseignant")
	@Operation(description = "Obtenir les classes visibles d'un enseignant", summary = "")
	@Tag(name = "Classe")
	public Response getVisibleByPersonnel(@QueryParam("personnel") long enseignantId, @QueryParam("ecole") long ecoleId,
			@QueryParam("annee") long anneeId) {
		try {
			List<ClasseDto> dtos = classeService.getClassesByEnseignantId(enseignantId, ecoleId, anneeId);
