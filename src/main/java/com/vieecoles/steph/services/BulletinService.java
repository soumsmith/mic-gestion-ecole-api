...
//		bul.setCodeEducateur(null);
	...
	public int handleSave_v2(String classe, String annee, String periode) {
		...
		try {
			...
			logger.info(String.format("Traitement terminé en %d secondes pour %d élèves", totalSeconds,
					context.moyenneParEleve.size()));
	...
	private BulletinProcessingContext preloadAllData(String classe, String annee, String periode) {
		...
		try {
			...
			logger.info(String.format("Calcul moyennes: %d secondes", (endMoyennes - startMoyennes) / 1_000_000_000));
			...
			// Le parallélisme avec DB peut causer des problèmes de connexions, deadlocks,
			// et transactions
			...
			List<Inscription> inscriptions = inscriptionService.getByClasseAndEcoleAndAnnee(
					firstEleve.getClasse().getId(), firstEleve.getClasse().getEcole().getId(), context.anneeId,
			...
			context.cepMap = cepList.stream().collect(Collectors.toMap(c -> c.getEleve().getId(), c -> c));

			logger.info("Chargement des absences...");
			List<AbsenceEleve> absences = absenceService.getListByAnneeAndPeriode(context.anneeId, context.periodeId);
			context.absenceMap = absences.stream().collect(Collectors.toMap(a -> a.getEleve().getId(), a -> a));

			// 4. Données matières et professeurs
			logger.info("Chargement des matières élèves...");
			context.cemList = classeEleveMatiereService.findByClasseAnneeAndPeriode(context.classeId, context.anneeId,
					context.periodeId);

			// Index optimisé : clé composite (eleveId + matiereId)
			context.cemByEleveAndMatiere = context.cemList.stream().collect(
					Collectors.toMap(cem -> cem.getEleve().getId() * 1000000L + cem.getMatiere().getId(), cem -> cem));

			logger.info("Chargement des professeurs...");
			context.professeurs = personnelMatiereClasseService.findProfesseursByClasse(context.anneeId,
					context.classeId);
			context.professeursMap = context.professeurs.stream().filter(p -> p.getMatiere() != null)
	...
	private Bulletin createBulletinOptimized(MoyenneEleveDto me, BulletinProcessingContext context) {
		...
		if (context.educateur != null && context.educateur.getPersonnel() != null) {
			...
			bulletin.setNomPrenomEducateur(
					context.educateur.getPersonnel().getNom() + " " + context.educateur.getPersonnel().getPrenom());
		}
		...
		if (absence != null) {
			bulletin.setHeuresAbsJustifiees(
					absence.getAbsJustifiee() != null ? absence.getAbsJustifiee().toString() : "0");
			bulletin.setHeuresAbsNonJustifiees(
					absence.getAbsNonJustifiee() != null ? absence.getAbsNonJustifiee().toString() : "0");
		} else {
			bulletin.setHeuresAbsJustifiees("0");
			bulletin.setHeuresAbsNonJustifiees("0");
		}
	...
	/**
	 * SAUVEGARDE SÉQUENTIELLE SÉCURISÉE AVEC GESTION TRANSACTIONNELLE Évite les
	 * problèmes de concurrence et de gestion des connexions DB
	 */
	...
	/**
	 * MISE À JOUR DES STATISTIQUES SÉCURISÉE Traitement par petits lots pour éviter
	 * les timeouts et problèmes de mémoire
	 */
	private void updateBulletinsWithStatistics(List<String> bulletinIds, List<Double> moyennes, int countNonClasses,
		...
		logger.info(String.format("Statistiques: Max=%.2f, Min=%.2f, Avg=%.2f, Effectif=%d, Non classés=%d", maxMoy,
				minMoy, avgMoy, effectif, countNonClasses));
	...
	/**
	 * SELECTION DES INFORMATIONS DE BULLETINS
	 */
