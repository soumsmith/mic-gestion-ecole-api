...
import com.vieecoles.services.etats.appachePoi.Annuels.EleveAffecteNonAffecteAnnuelsParClassePoiServices;
...
@ApplicationScoped
public class WordTempListAffecteNonAffectesAnnuelsProcessor {
    @Inject
    EntityManager em;
    @Inject
    EleveAffecteNonAffecteAnnuelsParClassePoiServices eleveAffecteParClassePoiServices ;
...
//Get classe
        ...
        for (int i = 0; i < paragraphs.size(); i++) {
            ...
            if (text.contains("Liste  nominative  des  élèves  et  résultats   scolaires(Annuels)")) {
        ...
        if (indexToInsert != -1) {
           ...
           // currentValue = MyEcole.getLibelle();
            for (eleveAffecteParClasseDtoAvecTousTrimestres eleve : elevAffectes) {  // Exemple de 3 lignes
                ...
                ensureCellCount(row, 14);  // Assurez-vous que chaque ligne a 16 cellules
               ...
               // setCellTextAndFontSize(row.getCell(10), eleve.getNumDecisionAffecte(), 10);
                ...
                setCellTextAndFontSize(row.getCell(11), String.valueOf(eleve.getMoyenAnnuel()), 10);
                setCellTextAndFontSize(row.getCell(12), String.valueOf(eleve.getRangAn()), 10);
               // setCellTextAndFontSize(row.getCell(17), String.valueOf(eleve.getClasseLibelle()), 10);
                setCellTextAndFontSize(row.getCell(13), eleve.getObservat(), 10);
